...
build_dat() {
	...
	cp -rv $SRCDIR/dat/geoip-only-cn-private.dat $TMP/
	...
	cp -rv $SRCDIR/dat/dlc.dat $TMP/
}

copyconf() {
	echo ">>> Copying config..."
	cd ${SRCDIR}/release/config
	if [[ $GOOS == "linux" ]]; then
		cp -rv *.json $TMP/
		cp -rv systemd $TMP/
